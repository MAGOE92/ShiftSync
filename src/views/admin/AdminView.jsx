          ...
          <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 13, flex: 1, overflowY: "auto" }}>
            ...
            {isAbs2
              ...
              : can("createPlan") && <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                <button style={{ ...btn("bl"), justifyContent: "center" }} onClick={() => { const dr = JSON.parse(JSON.stringify(scheds[planMo])); emps.forEach(e2 => { if (!dr[e2.id]) { const row = Array(days).fill("-"); (abs[e2.id] || []).forEach(({ day, type }) => { if (day >= 1 && day <= days) row[day - 1] = type; }); dr[e2.id] = row; } }); setDraft(dr); setPaint(shiftDefs[0]?.key || "-"); setEditMode(true); setSdSel(null); }}><Icon n="pencil" s={14} />Im Planer bearbeiten</button>
        ...
        {aTab === "sched" && <div>
          <div style={{ ...crd, marginBottom: 12 }}>
            <div style={{ display: "flex", gap: 10, alignItems: "flex-end", flexWrap: "wrap" }}>
              ...
              {baseSc && !editMode && can("createPlan") && <button style={btn("w")} onClick={() => { const d = JSON.parse(JSON.stringify(scheds[planMo])); emps.forEach(e => { if (!d[e.id]) { const row = Array(days).fill("-"); (abs[e.id] || []).forEach(({ day, type }) => { if (day >= 1 && day <= days) row[day - 1] = type; }); d[e.id] = row; } }); setDraft(d); setPaint(shiftDefs[0]?.key || "-"); setEditMode(true); }}><Icon n="pencil" s={15} />Bearbeiten</button>}
