...
export default function App() {
  ...
  const [nef, setNef] = useState({ first: "", last: "", lid: "", pin: "", pref: "any", role: "staff", workPct: 100, inPlan: true });
  ...
  const addEmp = async () => { const pinT = nef.pin.trim(); const first = nef.first.trim(), last = nef.last.trim(); if (!first || !last || !nef.lid.trim() || pinT.length < 4) { flash("er", "Vor- und Nachname, ID, PIN (≥4)"); return; } if (seatFull) { flash("er", `Sitzplatz-Limit erreicht (${seatLimit}). Upgrade nötig.`); return; } const lid = nef.lid.trim().toLowerCase(); if (emps.some(e => e.lid === lid)) { flash("er", "Login-ID vergeben"); return; } const { first: _f, last: _l, ...rest } = nef; const e = { id: rid(), ...rest, pin: pinT, name: `${first} ${last}`, lid, workPct: Number(nef.workPct) || 100 }; await saveData({ ...data, emps: [...emps, e] }); setNef({ first: "", last: "", lid: "", pin: "", pref: "any", role: "staff", workPct: 100, inPlan: true }); flash("ok", `${e.name} angelegt · PIN ${e.pin}`); };
  const saveEf = async () => { const first = ef.first?.trim(), last = ef.last?.trim(); if (!first || !last || !ef.lid?.trim()) { flash("er", "Vor- und Nachname, ID"); return; } const lid = ef.lid.trim().toLowerCase(); if (emps.some(e => e.id !== editE.id && e.lid === lid)) { flash("er", "ID vergeben"); return; } const { first: _f, last: _l, ...rest } = ef; const patch = { ...rest, name: `${first} ${last}`, lid, workPct: Number(ef.workPct) || 100 }; await saveData({ ...data, emps: emps.map(e => e.id === editE.id ? { ...e, ...patch } : e) }); if (me?.id === editE.id) setMe(p => ({ ...p, ...patch })); setEditE(null); flash("ok", "Gespeichert ✓"); };
